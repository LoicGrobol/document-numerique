...
- `[ab]` matche un caractère qui est soit `a` soit `b`
- Avec un tiret : `[a-c]` matche les lettres de `a` à `c`
...
- [^0-9] match tout sauf des chiffres
- [^A-Z] match tout sauf des majuscules sans diacritiques
...
- `[^<]*` matche une suite de caractères différents de `<`
...
- `([aeiouy][^aeiouy])+` matche une suite de couples voyelle-consonne.
