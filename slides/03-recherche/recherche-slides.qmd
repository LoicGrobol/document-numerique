...
subtitle: "Techniques de recherche avancées"
author:
...
1. Ouvrez-le avec l'éditeur de votre choix (Clic-droit sur le fichier + ouvrir avec)
1. Accéder à la fonction de recherche (en général
   <kbd>ctrl</kbd>+<kbd>F</kbd>/<kbd>R</kbd>/<kbd>H</kbd>).

## Compter

Dans le CLG, combien de fois apparaissent les mots :
...
- Règle générale : par défaut chaque caractère d'une expression correspond (*match*) à ce caractère :
  - Chercher `France 2` vous donne `France 2`, mais pas `France2` ni `France 3`
  - (en mode `Aa`) `madeleine` match `madeleine`, mais pas `Madeleine`
- Cas particuliers : les **métacaractères**
  - Quantificateurs
  - Opérateurs
  - Caractères spéciaux
  - Classes de caractères / propriétés unicode
...
- `pl?eur` matche `peur` et `pleur`
- `coup?s` matche `cous` et `coups`, mais pas `coupa`
- `joli*e` matche `jole`, `jolie`, `joliie`, `joliiie`…
- `jolie+` matche `jolie`, `joliee`, `jolieee`…

---

**Exercice** : à quelles chaînes de caractères correspondent les expressions suivantes ?
...
On les utilise le plus souvent avec des parenthèses pour contrôler leur portée
...
-  Le sens de certains de ces racourcis peut varier suivant les moteurs de regex, suivant leur support
...
1. Finissant par `er` en fin de ligne ?
1. Finissant par `er` ?
1. De 3 lettres finissant par `r` ?
1. De 4 lettres au plus commençant par `r` ?
1. Commençant par `a` et finissant par `er` ?
